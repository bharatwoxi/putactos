...
class CronController extends BaseController {
    ...
    /*
    *function Name: updateUserAge
    *Desc: Update User Age Based On Birth Date
   ...
   */
    public function updateUserAge(){
        ...
        foreach($users as $user){
            $dayDiff=floor((abs(strtotime(date("Y-m-d")) - strtotime($user->birth_date))/(60*60*24)));
            $years = ($dayDiff / 365) ; // days / 365 days
            $years = floor($years); // Remove all decimals
            DB::table('system_users')
                ->where('id', $user->id)
                ->update(array('current_age' => $years));
        }
    }
